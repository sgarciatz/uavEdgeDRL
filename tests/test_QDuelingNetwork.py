...
class test_QNetwork(unittest.TestCase):
    ...
    def test_create_q_network(self):
        ...
        n_obs = 32
        n_act = 4
        layers = [64, [64, 128], [128, 64], [64, 64], 64]
        device = "cpu"
        net = QDuelingNetwork(n_obs, n_act, layers, device)
        expected_layers = len(layers) * 2 -2
        ...
        self.assertEqual(net.value_output_layer.in_features, layers[-1])
        self.assertEqual(net.value_output_layer.out_features, 1)
        self.assertEqual(net.adv_output_layer.in_features, layers[-1])
        self.assertEqual(net.adv_output_layer.out_features, n_act)
